...
class ShouJiBaoPipeline(object):
    def __init__(self):
        ...
        if spider.name == 'shoujibao':
            try:
                ...
                if len(data) == 0:
                    ...
                    # self.cursor.execute("update spider_posts set url=\'%s\',title=\'%s\',author=\'%s\',content=\'%s\',status=1,has_img=%s,hash=\'%s\',source=\'%s\',create_time=%s,fetch_time=%s where hash=\'%s\'" % (
                    #     item['url'], item['title'], '', item['content'], item['has_img'], hash, source,
