...
class KankanPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if spider.name == 'kankannews':
            ...
            # self.cursor.execute( "update spider_posts set url=\'%s\',title=\'%s\',author=\'%s\',content=\'%s\',status=1,has_img=%s,hash=\'%s\',source=\'%s\',create_time=%s,fetch_time=%s where hash=\'%s\'"%(item['url'], item['title'], '',item['content'],item['has_img'], hash, source,item['create_time'], fetch_time,hash))
            #
            #
            # self.conn.commit()
            # self.change_item+=1
